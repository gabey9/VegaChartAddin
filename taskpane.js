...
export async function run() {
  ...
  // Validate 3 columns (Source, Value, Target)
  if (headers.length < 3) {
    console.error("Sankey requires 3 columns: Source, Value, Target");
    return;
  }

  // Filter and transform data - SankeyMATIC format
  const sankeyData = rows
    .filter(r => r[0] && r[2] && !isNaN(+r[1]))
    .map(r => ({
      key: { stk1: String(r[0]), stk2: String(r[2]) },
      doc_count: +r[1]
    }));

  if (sankeyData.length === 0) {
    console.error("No valid data found for Sankey chart");
    return;
  }

  // Complete Vega specification for multi-stage Sankey
  spec = {
    ...
    config: { view: { stroke: "transparent" }},
    view: { stroke: null },
    padding: { top: 60, bottom: 80, left: 80, right: 80 },
    data: [
      {
        name: "rawData",
        values: sankeyData,
    ...
    marks: [
      {
        ...
        encode: {
          update: {
            ...
            path: { field: "path" },
            strokeOpacity: {
              signal: "!groupSelector && !groupHover ? 0.4 : groupHover.stk1 == datum.stk1 || groupHover.stk2 == datum.stk2 ? 0.7 : 0.15"
            },
            zindex: {
              signal: "!groupSelector && !groupHover ? 0 : groupHover.stk1 == datum.stk1 || groupHover.stk2 == datum.stk2 ? 1 : 0"
            },
            tooltip: {
