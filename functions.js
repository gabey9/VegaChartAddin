...
function DUMBBELL(data) {
  return new Promise((resolve) => {
    try {
      ...
      if (headers.length < 3) {
        resolve("Error: Dumbbell chart requires 3 columns: Category, Period/Year, Value");
        return;
      }
      ...
      // Transform data for dumbbell chart (same as taskpane.js)
      const dumbellData = processedData.map(row => ({
      ...
      if (periods.length < 2) {
        resolve("Error: Dumbbell chart requires at least 2 different periods");
        return;
      }
